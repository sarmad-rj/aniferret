...
1. **Test-Driven Delivery:** Every backend endpoint and frontend page must include test coverage with a target of >=70% on core business logic.
2. **Strict Schema Validation:** All LLM and agent outputs must be validated through strict Pydantic models.
3. **Environment Security:** Never hardcode API keys or credentials; use `.env` files exclusively.
4. **Prompt Logging:** Log every major agentic scaffolding, prompt turn, and correction directly into `prompts.md`.
5. **Progressive Verification:** Use subagents to verify route registration, UI loading/error states, and API contracts after each file creation.